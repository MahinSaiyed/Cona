...
$conditions = ["p.is_active = 1", "p.is_on_sale = 1"];
$params = [];
...
// Get unique brands for sale items
$stmt = $db->prepare("SELECT DISTINCT brand FROM products WHERE is_active = 1 AND is_on_sale = 1 AND brand IS NOT NULL AND brand != '' ORDER BY brand");
$stmt->execute();
        ...
        <div class="filters">
            ...
            <div class="filter-group">
                ...
                <select name="price" class="filter-select" onchange="filterProducts()">
                    ...
                    <option value="0-2999" <?php echo $price_filter === '0-2999' ? 'selected' : ''; ?>>Under ₹2,999</option>
                    <option value="3000-5999" <?php echo $price_filter === '3000-5999' ? 'selected' : ''; ?>>₹3,000 - ₹5,999</option>
                    <option value="6000-9999" <?php echo $price_filter === '6000-9999' ? 'selected' : ''; ?>>₹6,000 - ₹9,999</option>
                    <option value="10000-99999" <?php echo $price_filter === '10000-99999' ? 'selected' : ''; ?>>₹10,000+</option>
                </select>
            </div>
        </div>

        <?php if (empty($products)): ?>
                <div style="text-align: center; padding: 5rem 0; background: var(--color-light-gray); margin: 2rem 0; border-radius: 8px;">
                    <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="var(--color-gray)" stroke-width="1" style="margin-bottom: 1.5rem; opacity: 0.5;">
                        <circle cx="11" cy="11" r="8"></circle>
                        <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
                        <line x1="11" y1="8" x2="11" y2="14"></line>
                        <line x1="8" y1="11" x2="14" y2="11"></line>
                    </svg>
                    <h3 style="font-size: 1.5rem; margin-bottom: 0.5rem;">No sale items found</h3>
                    <p style="color: var(--color-gray); margin-bottom: 2rem;">We couldn't find any sale items matching your filters.</p>
                    <button onclick="clearFilters()" class="btn btn-primary" style="padding: 0.75rem 2rem;">Clear All Filters</button>
                </div>
        <?php else: ?>
                <div class="grid grid-4">
                    <?php foreach ($products as $product):
                        ...
                        ?>
                            <div class="product-card" data-gallery='<?php echo json_encode($images); ?>'>
                                <div class="product-image">
                                    ...
                                    <div class="product-badges">
                                        <span class="badge-sale">
                                            <?php echo $discount > 0 ? $discount . '% OFF' : 'SALE'; ?>
                                        </span>
