<?php
// This file is duplicated for new-arrivals.php - filters products by is_new_arrival flag
...
$conditions = ["p.is_active = 1", "p.is_new_arrival = 1"];
$params = [];
...
// Get unique brands for new arrivals
$stmt = $db->prepare("SELECT DISTINCT brand FROM products WHERE is_active = 1 AND is_new_arrival = 1 AND brand IS NOT NULL AND brand != '' ORDER BY brand");
$stmt->execute();
    ...
    <div class="container">
        ...
        <div class="filters">
            ...
            <div class="filter-group">
                ...
                <select name="price" class="filter-select" onchange="filterProducts()">
                    ...
                    <option value="0-2999" <?php echo $price_filter === '0-2999' ? 'selected' : ''; ?>>Under ₹2,999
                    </option>
                    <option value="3000-5999" <?php echo $price_filter === '3000-5999' ? 'selected' : ''; ?>>₹3,000 -
                        ₹5,999</option>
                    <option value="6000-9999" <?php echo $price_filter === '6000-9999' ? 'selected' : ''; ?>>₹6,000 -
                        ₹9,999</option>
                    <option value="10000-99999" <?php echo $price_filter === '10000-99999' ? 'selected' : ''; ?>>₹10,000+
                    </option>
                </select>
            </div>
        </div>
        ...
        <?php if (empty($products)): ?>
            <div
                ...
                <h3 style="font-size: 1.5rem; margin-bottom: 0.5rem;">No new arrivals found</h3>
                <p style="color: var(--color-gray); margin-bottom: 2rem;">We couldn't find any new arrivals matching your
                    filters.</p>
